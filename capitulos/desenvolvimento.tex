...
\begin{algorithm}[H]
	\SetAlgoLined
	\Inicio{
		\Entrada{\textit{Dataset}, Nó raiz, Número de grupos}
		\Saida{Salva os grupos}
		Raiz = vértice raiz;
		...
		Salva os grupos no arquivo de saida;
	}
	\label{alg:main-aqui}
	\caption{\textsc{Algoritmo principal}}
\end{algorithm}
...
\begin{algorithm}[H]
	\SetAlgoLined
	\Inicio{
		\Entrada{Matriz de Adjacência, Nó raiz}
		\Saida{Arvore Geradora Mínima (AGM)}
	}
	\label{alg:main-aqui}
	\caption{Prim}
\end{algorithm}
...
\begin{algorithm}[H]
	\SetAlgoLined
	\Inicio{
		\Entrada{AGM, Número de Grupos}
		\Saida{Grupos separados}
	}
	\label{alg:main-aqui}
